...
func TestReadiness(t *testing.T) {
	...
	if resp.StatusCode != http.StatusServiceUnavailable {
		t.Errorf("NotReady Response.StatusCode = %d; want %d;", resp.StatusCode, http.StatusServiceUnavailable)
	}
	Ready()
	resp, err = http.Get(ts.URL)
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK {
		t.Errorf("Ready Response.StatusCode = %d; want %d;", resp.StatusCode, http.StatusOK)
	}
}
...
func TestLivenessChecks(t *testing.T) {
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusServiceUnavailable {
		t.Errorf("Response.StatusCode = %d; want %d;", resp.StatusCode, http.StatusServiceUnavailable)
	}
}
