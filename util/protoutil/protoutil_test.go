...
import (
	...
	"github.com/pthethanh/micro/util/protoutil"
)
...
func TestValidateFieldMask(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			if valid, npth := protoutil.IsValidFieldMask(c.path, v); valid != c.valid && npth != c.npath {
				t.Errorf("got valid=%t, path=%s, want valid=%t, path=%s", valid, npth, c.valid, c.path)
			}
		})
	}
}
...
func TestGetValidFieldMask(t *testing.T) {
	...
	paths := protoutil.GetValidFieldMask([]string{
		"name",
		"NextMeta.values",
		"meta.values",
		"address.location.Y",
